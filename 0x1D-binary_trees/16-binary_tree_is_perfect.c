 ...
 */
const binary_tree_t *obt_leaf(const binary_tree_t *tree)
{
	if (is_leaf(tree) == 1)
		return (tree);
	return (tree->left ? obt_leaf(tree->left) : obt_leaf(tree->right));
}
...
int binary_tree_is_perfect(const binary_tree_t *tree)
{
	if (tree == NULL)
		return (0);
	return (is_perfect_recursive(tree, depth(obt_leaf(tree)), 0));
}
